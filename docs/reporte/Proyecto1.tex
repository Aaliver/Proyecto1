...
\newcommand{\newItem}[3]{
  \item {\color{maincolor}\textbf{#1}}
    \begin{itemize}
      \item {\fontseries{sb}\selectfont Petición del cliente:} #2
      \item {\fontseries{sb}\selectfont Respuesta del servidor:} #3
    \end{itemize}
}
...
\newcommand{\uml}[3]{
    \begin{center}
        \begin{tcolorbox}[colback=maincolor!10, colframe=maincolor,
            colupper=black, fontupper=\small\ttfamily,
            boxrule=0.2mm, arc=2pt,
            left=3pt, right=3pt, top=1pt, bottom=1pt, width=2.5in,
            title={\centering\fontseries{sb}\selectfont #1},
